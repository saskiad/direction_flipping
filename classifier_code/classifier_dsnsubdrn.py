# Iman Wahle
# August 26 2019
# Classify while substituting drns for random dsns in input data
...
def classification( session_id, methods, N_iter, use_events, 
    ...
    drn_set = np.array(drn_set)
    dsn_set = np.array(dsn_set)
    N_drns = len(drn_set)
...
#     N_drns = len(drn_idx)
    ...
    for n_drns in range(N_drns+1):
        for n_iter in range(N_iter):
            ...
            chosen_neurons = np.copy(dsn_set)            
            if n_drns > 0:
                switch_idx = np.random.choice(range(N_drns), n_drns, replace=False)
                chosen_neurons[switch_idx] = drn_set[switch_idx]
            # st = time.time()
