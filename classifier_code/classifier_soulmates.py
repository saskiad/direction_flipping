# Iman Wahle
# August 26 2019
# Classify while substituting drns for analagous dsns in input data
...
drn_soulmates = np.load(path + "/drn_analog_code/drns.npy")
dsn_soulmates = np.load(path + "/drn_analog_code/soulmates.npy")
...
def classification( session_id, methods, use_events, 
    ...
    session_idx = np.where(sessions==session_id)[0][0]
    drn_ids = drn_soulmates[session_idx]
    dsn_ids = dsn_soulmates[session_idx]

    drn_set = []
    dsn_set = []
    for j in range(len(drn_ids)):
        drn_set.append(np.where(cells==drn_ids[j])[0][0])
        dsn_set.append(np.where(cells==dsn_ids[j])[0][0])
...
#     N_drns = len(drn_idx)
    ...
    bar = progressbar.ProgressBar(maxval=len(methods)*len(sets), \
    widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
    bar.start()
    barcnt = 0

    train_accs = np.zeros((len(sets), len(methods)))
    test_accs = np.zeros((len(sets), len(methods)))
    for ins,input_set in enumerate(sets):
                
        chosen_neurons = input_set # np.random.choice(input_set, n_drns+1, replace=True)
        # st = time.time()
        for m, method in enumerate(methods):
            ...
            train_accs[ins, m] = train_acc
            test_accs[ins, m] = test_acc
...
def main():
    ...
    folder = sys.argv[2]
    series = sys.argv[3]
    include_tf = bool(int(sys.argv[4]))
    methods = sys.argv[5:]

    xaccs = classification(session_id, methods, use_events=1, 
    ...
    fp = home + "/programs/braintv/workgroups/nc-ophys/Iman/" + \
        "direction_flipping/classifier_code/results/"
